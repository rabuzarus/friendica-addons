...
function jsxc_addon_get_client_secret(callback) {
	var client_secret = getPersistent('jappix-mini', 'client-secret');
	if (client_secret===null) {
		var div = document.getElementById("#jsxc-password-query-div");

		if (!div) {
			div = $('<div id="jsxc-password-query-div" style="position:fixed;padding:1em;background-color:#F00;color:#fff;top:50px;left:650px;">Retype your Friendica password for chatting:<br></div>');

			var input = $('<input type="password" id="jsxc-password-query-input">')
			div.append(input);

			var button = $('<input type="button" value="OK" id="jsxc-password-query-button">');
			div.append(button);
		...
		button.click(function(){
			var password = $("#jsxc-password-query-input").val();
			jsxc_addon_set_client_secret(password);
			div.remove();
...
function jsxc_addon_encrypt_password(password, callback) {
	jsxc_addon_get_client_secret(function(client_secret){
		...
		var encrypted_password = jsxc_addon_xor(client_secret, password);
...
function jsxc_addon_decrypt_password(encrypted_password, callback) {
	encrypted_password = decodeURI(encrypted_password);

	jsxc_addon_get_client_secret(function(client_secret){
		// xor password with secret
		var password = jsxc_addon_xor(client_secret, encrypted_password);
...
function jsxc_manage_roster(contacts, contacts_hash, autoapprove, autosubscribe) {
...
//
//        jsxc_manage_roster(contacts, contacts_hash, autoapprove, autosubscribe)
//    }
//
//    // decrypt password if necessary
//    if (encrypted)
//        jsxc_addon_decrypt_password(password, handler);
//    else
//        handler(password);
//}
